...
export class MagicLinkService {
  ...
  async generateMagicLink(options: MagicLinkOptions): Promise<MagicLinkResult> {
    ...
    try {
      ...
      const url = this.buildTargetUrl(linkType, token, options.customParams || {}, options.claimId, options.requirementTypes);
   ...
   */
  async sendMagicLink(options: SendMagicLinkOptions): Promise<{ 
    ...
    const message = customMessage || this.buildMessage(options.linkType, magicLink.shortUrl || magicLink.url, userName);
    ...
    try {
      ...
      return { magicLink, deliveryResult };
   ...
   */
  async trackAccess(token: string, userAgent?: string, ipAddress?: string): Promise<boolean> {
    try {
   ...
   */
  async getUserMagicLinkHistory(userId: number, options: {
    page?: number;
    limit?: number;
    linkType?: MagicLinkType;
    startDate?: Date;
  ...
  private verifyToken(token: string): { userId: number; linkType: MagicLinkType; timestamp: number } | null {
    try {
  ...
  private async sendViaSMS(phoneNumber: string, message: string, options: SendMagicLinkOptions) {
    // Import SMS service to send the message
  ...
  private async sendViaWhatsApp(phoneNumber: string, message: string, options: SendMagicLinkOptions) {
