...
export function DashboardPage() {
  ...
  const { data: metricsData, isLoading: metricsLoading } = useQuery({
    ...
    queryFn: () => apiClient.get<DashboardMetrics>(`/api/analytics/dashboard?startDate=${dateRange.start}&endDate=${dateRange.end}`),
  ...
  const { data: agentsData, isLoading: agentsLoading } = useQuery({
    ...
    queryFn: () => apiClient.get<AgentStatus[]>('/api/analytics/agent-status'),
  ...
  return (
    <div className="max-w-7xl mx-auto py-6 sm:px-6 lg:px-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        <MetricCard
          title="Total Calls"
          value={metrics?.callMetrics?.totalCalls || 0}
        ...
        <MetricCard
          ...
          value={`${metrics?.callMetrics?.completionRate || 0}%`}
        ...
        <MetricCard
          title="Queue Depth"
          value={metrics?.queueMetrics?.currentDepth || 0}
        ...
        <MetricCard
          title="Active Agents"
          value={`${metrics?.agentMetrics?.onlineAgents || 0}/${metrics?.agentMetrics?.totalAgents || 0}`}
          change="2 online"
          trend="neutral"
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        <div className="bg-white rounded-lg border border-gray-200 p-6">
          ...
          <div className="text-3xl font-bold text-gray-900">
            {metrics?.callMetrics?.totalTalkTime ? 
        ...
        <div className="bg-white rounded-lg border border-gray-200 p-6">
          ...
          <div className="text-3xl font-bold text-gray-900">
            {metrics?.callMetrics?.averageDuration ? 
        ...
        <div className="bg-white rounded-lg border border-gray-200 p-6">
          ...
          <div className="text-3xl font-bold text-gray-900">
            {metrics?.queueMetrics?.averageWaitTime ? 
      ...
      <div className="mt-8 grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-6">
          ...
          <div className="mt-4 space-y-2 text-sm text-yellow-700">
            {metrics?.queueMetrics?.currentDepth && metrics.queueMetrics.currentDepth > 50 && (
              <div>• Queue depth is high ({metrics.queueMetrics.currentDepth} users waiting)</div>
            )}
            {metrics?.callMetrics?.completionRate && metrics.callMetrics.completionRate < 70 && (
              <div>• Call completion rate is below target ({metrics.callMetrics.completionRate}%)</div>
            )}
            {metrics?.queueMetrics?.averageWaitTime && metrics.queueMetrics.averageWaitTime > 300 && (
              <div>• Average wait time is high ({Math.floor(metrics.queueMetrics.averageWaitTime / 60)} minutes)</div>
            )}
            {!metrics?.queueMetrics?.currentDepth && !metrics?.callMetrics?.completionRate && (
