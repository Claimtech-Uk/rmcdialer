...
export class HumeTTSService {
  ...
  async synthesizeText(text: string): Promise<HumeTTSChunk> {
    try {
      ...
      // Build utterance object (text, voice, etc. go inside utterances array)
      const utterance: any = {
        text: text.trim()
      };
      ...
      // Build request body with correct structure based on error analysis
      const requestBody = {
        utterances: [utterance],
        format: { type: 'wav' } // Format must be an object, not a string
      };

      console.log('🎤 Hume TTS request body:', JSON.stringify(requestBody, null, 2));
  ...
  /**
   * Create and save a custom voice (removed - was using deprecated HumeClient SDK)
   * TODO: Implement using direct REST API if needed in the future
   */
